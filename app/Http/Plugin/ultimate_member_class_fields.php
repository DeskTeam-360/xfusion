...
//							}
        ...
        public function display_view( $mode, $args ) {
            ...
            global $wpdb;
            $query = "SELECT * FROM wp_users WHERE user_nicename = '$user'";
            ...
            if ( UM()->options()->get( 'profile_empty_text' ) ) {
                ...
                if ( um_is_myprofile() ) {
                    ...
                    global $wpdb;
                    $query = "SELECT * FROM course_group_details";
                    ...
                    foreach ($q_list as $q) {
                        ...
                        $link_parent = strtolower(str_replace(' ', '-', $c_list[0]->course_title));
                        $link_child = strtolower(str_replace(' ', '-', $c_list[0]->page_title));
                        ...
                        $data = $wpdb->get_row($wpdb->prepare("SELECT * FROM wp_gf_form_meta WHERE form_id = %d", $form_id));
                        //$data_meta = json_decode($data->display_meta);
                        // print_r($display_meta->title);
                        ...
                        $data_fields = $fields->fields;

                        $modal_id = str_replace(' ', '-', strtolower($c_list[0]->page_title));

                        // print_r($data_fields)
                        if ($entry_id) {
                            $output .= '<div class="note-column" id="open'.$modal_id.'"><span>'. $c_list[0]->page_title .'</span></div>';
                        } else {
                            $output .= '<div class="note-column" style="color: red; pointer-events: none;"><span>'. $c_list[0]->page_title .'</span></div>';
                        }


                        $output .= '
                        <div id="my'.$modal_id.'" class="modal">
                            <div class="modal-content">
                                <span class="close" id="close'.$modal_id.'">&times;</span>
                                <h2>'. $c_list[0]->page_title .'</h2>
                                <table>
                                    <thead>
                                        <tr>
                                            <th>Question</th>
                        ...
                        $output .= '</tbody>
                                ...
                                <div style="text-align: right; margin-top: 10px;">
                                    <a class="custom-btn" href="/'. $link_parent .'/'. $link_child .'/?dataId='. $entry_id .'">Details</a>
                                </div>
                            </div>
                        </div>';
                        ...
                        $output .= '
                        <script>
                            document.getElementById("open'.$modal_id.'").onclick = function() {
                                document.getElementById("my'.$modal_id.'").style.display = "block";
                            }
                            document.getElementById("close'.$modal_id.'").onclick = function() {
                                document.getElementById("my'.$modal_id.'").style.display = "none";
                            }

                            // Listen for outside click to close modal
                            window.onclick = function(event) {
                                if (event.target == document.getElementById("my'.$modal_id.'")) {
                                    document.getElementById("my'.$modal_id.'").style.display = "none";
                                }
                            }
                        </script>';
