    ...
    class Fields
    {
        ...
        public function delete_field_from_form($id, $form_id)
        {
            ...
            if (isset($fields[$id])) {
                ...
                if (!is_array($condition_fields)) $condition_fields = [];
        ...
        public function delete_field_from_db($id)
        {
            ...
            if (isset($fields[$id])) {
                ...
                foreach ($directories as $directory_id) {
                    ...
                    $directory_search_fields = !is_array($directory_search_fields) ? [] : $directory_search_fields;
                    $directory_search_fields = array_values(array_diff($directory_search_fields, [$id]));
                    ...
                    $directory_reveal_fields = !is_array($directory_reveal_fields) ? [] : $directory_reveal_fields;
                    $directory_reveal_fields = array_values(array_diff($directory_reveal_fields, [$id]));
                    ...
                    $directory_tagline_fields = !is_array($directory_tagline_fields) ? [] : $directory_tagline_fields;
                    $directory_tagline_fields = array_values(array_diff($directory_tagline_fields, [$id]));
                    ...
                    $directory_sorting_fields = !is_array($directory_sorting_fields) ? [] : $directory_sorting_fields;
         ...
         */
        private function add_field_from_list($global_id, $form_id, $position = [])
        {
            $fields = UM()->query()->get_attr('custom_fields', $form_id);
         ...
         * @param array $position
         */
        private function add_field_from_predefined($global_id, $form_id, $position = [])
        {
            $fields = UM()->query()->get_attr('custom_fields', $form_id);
        ...
        function field_value($key, $default = false, $data = null)
        {
            ...
            } else if (true === $this->editing && um_user($key)) {
            ...
            } else if (true === $this->viewing && (um_user($key) || isset($data['show_anyway']))) {
            ...
            } else if (isset($value) && is_array($value) && !count($value)) {
                $value = '';
            } else if (!isset($value)) {
                $value = '';
            }
        ...
        public function dropdown_options_source_blacklist()
        {
            ...
            $blacklist = !empty($list['internal']) ? $list['internal'] : [];
        ...
        function get_option_value_from_callback($value, $data, $type)
        {
            ...
            if (in_array($type, ['select', 'multiselect']) && !empty($data['custom_dropdown_options_source'])) {
                ...
                if ($has_custom_source) {
                    ...
                    $opts = apply_filters("um_get_field__{$data['metakey']}", []);
                    $arr_options = array_key_exists('options', $opts) ? $opts['options'] : [];
                ...
                if ($has_custom_source || function_exists($data['custom_dropdown_options_source'])) {
                    if ($type == 'select') {
                        ...
                        } else if (!empty($data['default']) && empty($arr_options[$value])) {
        ...
        public function get_options_from_callback($data, $type)
        {
            ...
            if (!empty($data['custom_dropdown_options_source']) && in_array($type, ['select', 'multiselect'], true)) {
                if ($this->is_source_blacklisted($data['custom_dropdown_options_source'])) {
                    return $arr_options;
                }
        ...
        public function get_label($key)
        {
            $label = '';
            $fields = UM()->builtin()->all_user_fields;
            $field_data = array_key_exists($key, $fields) ? $fields[$key] : [];
        ...
        public function get_fields()
        {
            ...
            if (empty($this->fields[$this->set_id])) {
                 ...
                 */
                $this->fields[$this->set_id] = apply_filters('um_get_form_fields', [], $this->set_id);
            }
        ...
        public function get_restricted_fields_for_edit($_um_profile_id = false)
        {
            ...
            $arr_restricted_fields = ['user_email', 'username', 'user_login', 'user_password', '_um_last_login', 'user_registered'];
            /**
         ...
         * @throws Exception
         */
        public function edit_field($key, $data, $rule = false, $args = [])
        {
            ...
            switch ($type) {
                ...
                case 'select':
                case 'radio':
                    $form_key = str_replace(['role_select', 'role_radio'], 'role', $key);
                    $field_id = $form_key;
                    break;
                default:
                    $field_id = '';
            ...
            switch ($type) {
                ...
                case 'password':
                    ...
                    if ('single_user_password' === $key) {
                        ...
                        if ('login' !== $this->set_mode && !empty($data['force_confirm_pass'])) {
                            ...
                            } else if (!empty($placeholder) && !isset($data['label'])) {
                ...
                case 'spacing':
                    ...
                    if (array_key_exists('spacing', $data)) {
                        $field_style = ['height' => $data['spacing']];
                    }
                ...
                case 'divider':
                    ...
                    if (!empty($border_style)) {
                        $field_style = ['border-bottom' => $border_style];
                    }
                ...
                case 'image':
                    ...
                    if (in_array($key, ['profile_photo', 'cover_photo'], true)) {
                        $field_value = '';
                    } else {
                        $field_value = $this->field_value($key, $default, $data);
                    }
...
//							}
                    ...
                    if (empty($disabled)) {
                        ...
                        } else if (is_array($data['allowed_types'])) {
         ...
         * @return array
         */
        function get_available_roles($form_key, $options = [])
        {
            if ('role' !== $form_key) {
                return $options;
            }
        ...
        public function view_field($key, $data, $rule = false)
        {
            ...
            switch ($type) {
                ...
                case 'rating':
                    ...
                    $number = 5;
                    if (array_key_exists('number', $data) && in_array(absint($data['number']), [5, 10], true)) {
                        $number = $data['number'];
                    }
                    ob_start();
                    ?>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    for (var i = 0; i < acc.length; i++) {
         ...
         *
         */
        function get_atts($key, $classes, $conditional, $data, $field_style = [])
        {
            ...
            if (!empty($field_style) && is_array($field_style)) {
                ...
                $field_atts['style'] = [$arr_inline_style];
            }
