...
<div class="text-dark dark:text-darklink">
    ...
    <fieldset wire:loading.attr="disabled" wire:loading.class="opacity-60" wire:target="save" class="min-w-0 border-0 p-0 transition-opacity">
        ...
        <p class="mb-4 text-sm text-dark/70 dark:text-darklink">
        ...
        @else
            <div class="max-h-[32rem] space-y-2 overflow-y-auto rounded border border-border bg-white p-3 [color-scheme:light] dark:[color-scheme:dark] dark:bg-darkgray/30 dark:border-darkborder">
                @foreach($fields as $f)
                    @php
                        $_id = (int) $f['id'];
